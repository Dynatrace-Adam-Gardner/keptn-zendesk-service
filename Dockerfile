...
RUN GOOS=linux go build -ldflags '-linkmode=external' $BUILDFLAGS -v -o zendesk-service
...
COPY --from=builder /src/zendesk-service/zendesk-service /zendesk-service
